...
class GRU(nn.Module):
    def __init__(self, channel, h_w, kernel_size=3, stride=1, padding=1):
        ...
        self.conv_2 = nn.Conv2d(channel, channel, kernel_size=kernel_size, stride=stride, padding=padding)
        self.conv_3 = nn.Conv2d(channel, channel, kernel_size=kernel_size, stride=stride, padding=padding)

    def forward(self, g, l):
        cat = self.conv_1(torch.cat((g, l), dim=1))
        Z, R = torch.chunk(cat, 2, dim=1)
        Z = torch.sigmoid(Z)
        R = torch.sigmoid(R)

        return Z * self.conv_2(g) + R * self.conv_3(l)
...
class Rainformer(nn.Module):
    def __init__(self, input_channel, hidden_dim, output_channel, downscaling_factors, layers, heads, head_dim, window_size, relative_pos_embedding):
        super(Rainformer, self).__init__()
        ...
        self.stage4 = StageModule(in_channels=hidden_dim * 4, hidden_dimension=hidden_dim * 8, layers=layers[3],
                                  downscaling_factor=downscaling_factors[3], num_heads=heads[3], head_dim=head_dim,
                                  window_size=window_size, relative_pos_embedding=relative_pos_embedding, h_w=[4, 4])
    ...
    def forward(self, x):
        ...
        x8 = torch.reshape(x8, [b, -1, c, h, w])
        x8 = x8.view(b, -1, c, h, w)
